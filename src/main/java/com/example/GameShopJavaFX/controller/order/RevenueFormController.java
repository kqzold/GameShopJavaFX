...
public class RevenueFormController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        private void initComboBoxes() {
            ...
            cbDay.getItems(FXCollections.observableArrayList(days));

            List<String> months = new ArrayList<>();
            for (int i = 1; i <= 12; i++) {
                months.add(String.valueOf(i));
            }
            cbMonth.getItems(FXCollections.observableArrayList(months));

            List<String> years = new ArrayList<>();
            for (int i = 2023; i <= 2025; i++) {
                years.add(String.valueOf(i));
            }
            cbYear.getItems(FXCollections.observableArrayList(years));
            cbYearMonth.getItems(FXCollections.observableArrayList(years));
        }
        ...
        private void calculateRevenue() {
            ...
            try {
                ...
                lblRevenue.setText("Ошибка при расчете выручки: " + e.getMessage());
            }
        }
    }
